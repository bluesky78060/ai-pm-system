...
// biome-ignore format: type-level dynamic import must stay on one line (TS1005 if wrapped)
...
describe('TaskService.updateStatus', () => {
	...
	it('throws when task does not exist', async () => {
		...
		await expect(svc.updateStatus('nonexistent-id', 'in_progress')).rejects.toThrow(
			/태스크를 찾을 수 없습니다/,
		);
	});
});
...
describe('TaskService.setPriority', () => {
	...
	it('throws when task does not exist', async () => {
		...
		await expect(svc.setPriority('nonexistent-id', 3, 'reason')).rejects.toThrow(
			/태스크를 찾을 수 없습니다/,
		);
	});
});
...
describe('TestService.reportTestResult', () => {
	...
	it('throws when task does not exist', async () => {
		...
		await expect(testSvc.reportTestResult('nonexistent-id', 'pass')).rejects.toThrow(
			/태스크를 찾을 수 없습니다/,
		);
	});
});
