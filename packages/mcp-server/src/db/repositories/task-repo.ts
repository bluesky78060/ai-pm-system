...
export class TaskRepository {
	...
	async findByEpic(epicId: string): Promise<Task[]> {
		const { rows } = await getPool().query(
			'SELECT * FROM tasks WHERE epic_id = $1 ORDER BY priority, created_at',
			[epicId],
		);
		return rows as Task[];
	}
	...
	async findByStatus(status: string, epicId?: string): Promise<Task[]> {
		if (epicId) {
			const { rows } = await getPool().query(
				'SELECT * FROM tasks WHERE status = $1 AND epic_id = $2 ORDER BY priority',
				[status, epicId],
			);
			return rows as Task[];
		}
		const { rows } = await getPool().query(
			'SELECT * FROM tasks WHERE status = $1 ORDER BY priority',
			[status],
		);
		return rows as Task[];
	}
	...
	async findByParent(parentId: string): Promise<Task[]> {
		const { rows } = await getPool().query(
			'SELECT * FROM tasks WHERE parent_id = $1 ORDER BY priority, created_at',
			[parentId],
		);
		return rows as Task[];
	}
	...
	async findAll(filters?: {
		...
		if (filters?.assignee) {
			conditions.push(`t.assignee = $${idx++}`);
		...
		const { rows } = await getPool().query(
			`SELECT t.* FROM tasks t ${where} ORDER BY t.priority, t.created_at`,
			values,
		);
		return rows as Task[];
	}
	...
	async findByTicketCode(ticketCode: string): Promise<Task | undefined> {
		const { rows } = await getPool().query('SELECT * FROM tasks WHERE ticket_code = $1', [
			ticketCode,
		]);
		return rows[0] as Task | undefined;
	}
	...
	async create(data: {
		...
		if (data.epic_id) {
			const {
				...
				'SELECT COALESCE(MAX(seq), 0) + 1 as next FROM tasks WHERE epic_id = $1',
				[data.epic_id],
			);
			seq = seqRow.next;
	...
	async updateStatus(id: string, status: string, _notes?: string): Promise<Task> {
		...
		await getPool().query('UPDATE tasks SET status = $1, completed_at = $2 WHERE id = $3', [
			status,
			completedAt,
			id,
		]);
	...
	async getDependencies(taskId: string): Promise<TaskDependency[]> {
		const { rows } = await getPool().query('SELECT * FROM task_dependencies WHERE task_id = $1', [
			taskId,
		]);
	...
	async getDependents(taskId: string): Promise<TaskDependency[]> {
		const { rows } = await getPool().query(
			'SELECT * FROM task_dependencies WHERE depends_on = $1',
			[taskId],
		);
		return rows as TaskDependency[];
	}
