...
export class FixAttemptRepository {
	...
	async findByTask(taskId: string): Promise<FixAttempt[]> {
		const { rows } = await getPool().query(
			'SELECT * FROM fix_attempts WHERE task_id = $1 ORDER BY attempt_number DESC',
			[taskId],
		);
	...
	async getLatestAttemptNumber(taskId: string): Promise<number> {
		const {
			...
			'SELECT COALESCE(MAX(attempt_number), 0) as max_attempt FROM fix_attempts WHERE task_id = $1',
			[taskId],
		);
