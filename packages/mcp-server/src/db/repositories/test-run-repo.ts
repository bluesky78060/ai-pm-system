...
export class TestRunRepository {
	...
	async findByTask(taskId: string): Promise<TestRun[]> {
		const { rows } = await getPool().query(
			'SELECT * FROM test_runs WHERE task_id = $1 ORDER BY run_number DESC, created_at DESC',
			[taskId],
		);
	...
	async getLatestRunNumber(taskId: string): Promise<number> {
		const {
			...
			'SELECT COALESCE(MAX(run_number), 0) as max_run FROM test_runs WHERE task_id = $1',
			[taskId],
		);
		return row.max_run;
	}
