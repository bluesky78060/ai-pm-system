...
export class EpicRepository {
  async findByProject(projectId: string): Promise<Epic[]> {
    const { rows } = await getPool().query(
      'SELECT * FROM epics WHERE project_id = $1 ORDER BY order_index, priority', [projectId]
    );
    return rows as Epic[];
  }
  ...
  async findBySeq(projectId: string, seq: number): Promise<Epic | undefined> {
    const { rows } = await getPool().query(
      'SELECT * FROM epics WHERE project_id = $1 AND seq = $2', [projectId, seq]
    );
    return rows[0] as Epic | undefined;
  }

  async create(data: { project_id: string; title: string; description?: string; priority?: number }): Promise<Epic> {
    const pool = getPool();
    const id = uuid();
    const { rows: [orderRow] } = await pool.query(
      'SELECT COALESCE(MAX(order_index), -1) + 1 as next FROM epics WHERE project_id = $1', [data.project_id]
    );
    const { rows: [seqRow] } = await pool.query(
      'SELECT COALESCE(MAX(seq), 0) + 1 as next FROM epics WHERE project_id = $1', [data.project_id]
    );
    await pool.query(
      'INSERT INTO epics (id, project_id, title, description, priority, order_index, seq) VALUES ($1, $2, $3, $4, $5, $6, $7)',
      [id, data.project_id, data.title, data.description ?? null, data.priority ?? 3, orderRow.next, seqRow.next]
    );
    return (await this.findById(id))!;
  }

  async update(id: string, data: Partial<Pick<Epic, 'title' | 'description' | 'status' | 'priority' | 'order_index'>>): Promise<Epic> {
    const pool = getPool();
    const fields: string[] = [];
