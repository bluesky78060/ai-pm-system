...
export class AutomationRepository {
  async create(data: Omit<AutomationRule, 'created_at'>): Promise<AutomationRule> {
    ...
    `, [
      data.id, data.project_id, data.name, data.trigger_event,
      data.condition ?? null, data.action_type, data.action_config ?? null, data.enabled,
    ]);
  ...
  async findByProject(projectId: string): Promise<AutomationRule[]> {
    const { rows } = await getPool().query(
      'SELECT * FROM automation_rules WHERE project_id = $1 ORDER BY created_at', [projectId]
    );
  ...
  async update(id: string, data: Partial<Pick<AutomationRule, 'name' | 'condition' | 'action_config' | 'enabled'>>): Promise<AutomationRule> {
    const fields: string[] = [];
    const values: unknown[] = [];
    let idx = 1;
    if (data.name !== undefined) { fields.push(`name = $${idx++}`); values.push(data.name); }
    if (data.condition !== undefined) { fields.push(`condition = $${idx++}`); values.push(data.condition); }
    if (data.action_config !== undefined) { fields.push(`action_config = $${idx++}`); values.push(data.action_config); }
    if (data.enabled !== undefined) { fields.push(`enabled = $${idx++}`); values.push(data.enabled); }
    if (fields.length === 0) return (await this.findById(id))!;
    values.push(id);
    await getPool().query(`UPDATE automation_rules SET ${fields.join(', ')} WHERE id = $${idx}`, values);
