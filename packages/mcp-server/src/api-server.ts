...
async function seedIfEmpty() {
	...
	const epics = [
		...
		{
			title: '인 프로세스 서브에이전트',
			description: 'MCP 서버 내 자동화 서브에이전트',
			priority: 1,
		},
	];
	...
	console.log(
		`[Seed] Project "${project.name}" (${project.code}) created with ${epics.length} epics`,
	);
}
...
app.get(
	'/api/projects',
	wrapAsync(async () => ({ projects: await projectService.getAll() })),
);

app.post(
	'/api/projects',
	wrapAsync(async (req) => projectService.create(req.body)),
);
...
app.get(
	...
	wrapAsync(async (req) => {
		...
		if (!project) {
			console.error(`[API] Project not found: ${req.params.id as string}`);
...
app.get(
	...
	wrapAsync(async (req) => contextService.getProjectStatus(req.params.id as string)),
);

app.get(
	'/api/projects/:id/context',
	wrapAsync(async (req) => contextService.getSessionContext(req.params.id as string)),
);

app.get(
	'/api/projects/:id/blocking',
	wrapAsync(async (req) => contextService.getBlockingAnalysis(req.params.id as string)),
);

app.get(
	'/api/projects/:id/priority-suggestions',
	wrapAsync(async (req) =>
...
app.get(
	'/api/projects/:id/assignment-recommendations',
	wrapAsync(async (req) => autoAssignmentService.assignRecommendations(req.params.id as string)),
);
...
app.post(
	'/api/tasks',
	wrapAsync(async (req) => taskService.create({ ...req.body, created_by: 'human' })),
);
...
app.get(
	...
	wrapAsync(async (req) => {
		...
		if (!task) {
			console.error(`[API] Task not found: ${req.params.id as string}`);
...
app.get(
	'/api/tasks/:id/priority-analysis',
	wrapAsync(async (req) => priorityRecommendationService.analyzePriority(req.params.id as string)),
);

app.get(
	'/api/tasks/:id/assignee-suggestion',
	wrapAsync(async (req) => autoAssignmentService.suggestAssignee(req.params.id as string)),
);
...
app.post(
	'/api/tasks/:id/decompose',
	wrapAsync(async (req) => taskService.decompose(req.params.id as string, req.body.subtasks)),
);
...
// === Test & Fix routes ===
app.get(
	'/api/tasks/:id/fix-history',
	wrapAsync(async (req) => testService.getFixHistory(req.params.id as string)),
);

app.post(
	'/api/tasks/:id/test-runs',
	wrapAsync(async (req) => testService.runTests(req.params.id as string, req.body.results)),
);

app.post(
	'/api/tasks/:id/test-result',
	wrapAsync(async (req) =>
...
app.post(
	...
	wrapAsync(async (req) => testService.escalateToHuman(req.params.id as string, req.body.reason)),
);
...
app.get(
	...
	wrapAsync(async (req) => {
		const task = await taskService.getById(req.params.id as string);
		if (!task) {
			console.error(`[API] Task not found: ${req.params.id}`);
			throw new Error('태스크를 찾을 수 없습니다');
		}
...
// === GitHub routes ===
app.post(
	'/api/tasks/:id/link-pr',
	wrapAsync(async (req) => githubService.linkPrToTask(req.params.id as string, req.body.pr_url)),
);

app.get(
	'/api/tasks/:id/pr-status',
	wrapAsync((req) => githubService.getPrStatus(req.params.id as string)),
);

app.post(
	'/api/tasks/:id/github-issue',
	wrapAsync(async (req) =>
...
app.get(
	...
	wrapAsync(async (req) => {
		...
		const allActivities = (
			await Promise.all(taskIds.map((tid) => activityRepo.findByTask(tid, limit)))
		).flat();
		allActivities.sort(
...
app.get(
	...
	wrapAsync(async (req) =>
		exportService.exportProject(
			req.params.id as string,
			(req.query.format as 'json' | 'csv') || 'json',
		),
	),
);
...
// === Analysis routes ===
app.get(
	'/api/projects/:id/analysis/:type',
	wrapAsync(async (req) => {
		const type = req.params.type as string;
		switch (type) {
			case 'daily_report':
				return analysisService.dailyReport(req.params.id as string);
			case 'bottleneck':
				return analysisService.bottleneck(req.params.id as string);
			case 'velocity':
				return analysisService.velocity(req.params.id as string);
			default:
...
// === Workload routes ===
app.get(
	'/api/projects/:id/workload',
	wrapAsync(async (req) => workloadService.getWorkloadByAssignee(req.params.id as string)),
);

app.get(
	'/api/projects/:id/workload/alerts',
	wrapAsync(async (req) => workloadService.getOverloadAlerts(req.params.id as string)),
);
...
app.patch(
	'/api/automation/:ruleId/toggle',
	wrapAsync(async (req) => automationService.toggleRule(req.params.ruleId as string)),
);
...
app.get(
	'/api/notification-settings/:eventType',
	wrapAsync(async (req) => {
		const userId = (req.query.user_id as string) || 'ai';
...
app.post(
	...
	wrapAsync(async (req) => templateService.applyTemplate(req.params.id as string, req.body)),
);

// === Seed demo activities for a task ===
app.post(
	'/api/tasks/:id/seed-activities',
	wrapAsync(async (req) => {
		const taskId = req.params.id as string;
		const task = await taskService.getById(taskId);
		if (!task) {
			console.error(`[API] Task not found: ${taskId}`);
			throw new Error('태스크를 찾을 수 없습니다');
		}
