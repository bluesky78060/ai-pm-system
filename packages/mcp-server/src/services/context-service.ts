...
import type { ProjectStatusOutput, SessionContext, Task } from '../types/index.js';
...
export class ContextService {
	async getSessionContext(projectId: string): Promise<SessionContext> {
		...
		if (!project) {
			console.error(`[ContextService] Project not found: ${projectId}`);
		...
		const testing = allTasks.filter(
			(t) => t.status === 'testing' || t.status === 'fixing' || t.status === 'review',
		);
	...
	async getBlockingAnalysis(projectId: string): Promise<BlockingAnalysis> {
		...
		const inProgressTasks = await taskRepo.findAll({
		...
		if (blockedTasks.length === 0 && delayedTasks.length === 0) {
			...
			if (blockedTasks.length > 0) {
				...
				if (maxImpact && maxImpact.blockedDependents.length > 0) {
					parts.push(
						`가장 영향이 큰 블로킹: '${maxImpact.task.title}' (${maxImpact.blockedDependents.length}개 태스크 대기)`,
					);
				}
			}
