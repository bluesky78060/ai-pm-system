...
export class ExportService {
	...
	async exportProject(
		...
		format: 'json' | 'csv',
	): Promise<ExportData | { format: 'csv'; data: Record<string, string> }> {
		// Resolve project
	...
	async importProject(data: ExportData, options?: { overwrite?: boolean }): Promise<ImportResult> {
		...
		try {
			...
			const existingProject = data.project.code
				? await projectRepo.findByCode(data.project.code)
				: undefined;

			if (existingProject) {
				if (!options?.overwrite) {
					throw new Error(
						`프로젝트 코드 '${data.project.code}'가 이미 존재합니다. overwrite 옵션을 사용하세요.`,
					);
				}
