...
export class ExportService {
  ...
  async exportProject(projectIdOrCode: string, format: 'json' | 'csv'): Promise<ExportData | { format: 'csv'; data: Record<string, string> }> {
    ...
    for (const task of tasks) {
      const taskDeps = await taskRepo.getDependencies(task.id);
    ...
    for (const task of tasks) {
      const taskActivities = await activityRepo.findByTask(task.id);
      activities.push(...taskActivities);
    }

    // Fetch test runs for all tasks
    const testRuns: TestRun[] = [];
    for (const task of tasks) {
      const taskTestRuns = await testRunRepo.findByTask(task.id);
      testRuns.push(...taskTestRuns);
    }

    // Fetch fix attempts for all tasks
    const fixAttempts: FixAttempt[] = [];
    for (const task of tasks) {
      const taskFixAttempts = await fixAttemptRepo.findByTask(task.id);
      fixAttempts.push(...taskFixAttempts);
    }
