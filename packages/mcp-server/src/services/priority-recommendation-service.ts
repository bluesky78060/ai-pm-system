...
async function resolveTask(idOrCode: string): Promise<Task> {
	...
	if (!task) {
		console.error(`[PriorityRecommendationService] Task not found: ${idOrCode}`);
...
export class PriorityRecommendationService {
	...
	async suggestPriorityAdjustments(projectId: string): Promise<ProjectPrioritySuggestion> {
		...
		const summary = {
			high_priority_needed: suggestions.filter((s) => s.suggested_priority < s.current_priority)
				.length,
			low_priority_needed: suggestions.filter((s) => s.suggested_priority > s.current_priority)
				.length,
	...
	private async calculatePriorityScore(
		task: Task,
		context: Awaited<ReturnType<typeof this.gatherTaskContext>>,
	) {
		let score = 50; // 기본 점수
