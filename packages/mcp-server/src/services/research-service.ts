...
export type ResearchPurpose = 'library_compare' | 'security_audit' | 'best_practice' | 'debugging';
...
function buildUserPrompt(topic: string, context: string | undefined): string {
	...
	lines.push('- 참고한 외부 자료는 본문 끝에 "## 참고 자료" 섹션 또는 마크다운 링크로 명시하세요');
	return lines.join('\n');
}
...
function classifyGeminiError(
	...
	if (
		isNetworkMessage ||
		isNetworkCode ||
		(errName === 'TypeError' && lowerMessage.includes('fetch'))
	) {
		return {
			code: 'NETWORK_ERROR',
