...
export class NotificationService {
  ...
  async notifyStatusChange(context: NotificationContext): Promise<void> {
    ...
    const shouldNotify = newStatus === 'done' || newStatus === 'blocked' ||
    ...
    if (extra?.epic_progress) {
      ...
      fields.push({ type: 'mrkdwn', text: `*에픽 진행률:*\n${ep.completed}/${ep.total} (${ep.rate}%)` });
    }
   ...
   */
  async notifyTestComplete(task: Task, testResults?: { build?: string; test?: string }): Promise<void> {
    if (!this.enabled) return;
  ...
  private async sendMessage(message: SlackMessage): Promise<void> {
    ...
    for (let attempt = 1; attempt <= this.retryCount; attempt++) {
      try {
        ...
        // 재시도 불가능한 에러 (4xx)
        console.error(`[NotificationService] Slack webhook error: ${response.status} ${response.statusText}`);
        return;
      } catch (error) {
