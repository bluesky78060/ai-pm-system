...
async function resolveTask(idOrCode: string): Promise<Task> {
	...
	if (!task) {
		console.error(`[AutoAssignmentService] Task not found: ${idOrCode}`);
...
export class AutoAssignmentService {
	...
	async assignRecommendations(projectId: string): Promise<ProjectAssignmentRecommendation> {
		...
		const unassignedTasks = tasks.filter(
			(t) => !t.assignee || t.assignee === '' || t.assignee === 'AI',
		);
		...
		// 요약 통계
		const summary = {
			executor_needed: recommendations.filter((r) => r.suggested_assignee.startsWith('executor'))
				.length,
			designer_needed: recommendations.filter((r) => r.suggested_assignee.startsWith('designer'))
				.length,
			reviewer_needed: recommendations.filter(
				(r) =>
					r.suggested_assignee.startsWith('code-reviewer') ||
					r.suggested_assignee.startsWith('security-reviewer'),
			).length,
			other_needed: recommendations.filter(
				(r) =>
