...
export class WorkloadService {
	async getWorkloadByAssignee(projectId: string): Promise<WorkloadAnalysis> {
		...
		const activeTasks = allTasks.filter((t) => t.status === 'in_progress' || t.status === 'done');
		...
		for (const [assignee, tasks] of assigneeMap) {
			...
			const estimatedHours = inProgressTasks.reduce((sum, t) => sum + (t.estimated_hrs || 0), 0);
		...
		const avgWorkload =
			workloads.length > 0 ? workloads.reduce((a, b) => a + b) / workloads.length : 0;
