...
function getOctokit(): Octokit {
  ...
  if (!token) {
    throw new Error('GITHUB_TOKEN 환경변수가 설정되지 않았습니다. GitHub 연동을 위해 Personal Access Token을 설정하세요.');
  }
...
function parseRepo(githubRepo: string): { owner: string; repo: string } {
  ...
  if (parts.length !== 2) throw new Error(`잘못된 GitHub 저장소 형식: ${githubRepo} (owner/repo 형식이어야 합니다)`);
...
export class GitHubService {
  ...
  async getPrStatus(taskId: string): Promise<{
    ...
    if (!prMatch) {
      return {
        task,
        pr: { url: task.github_pr, state: 'unknown', title: '', mergeable: null, reviewDecision: null, checks: 'unknown' },
    ...
    try {
      ...
      return {
        task,
        pr: { url: task.github_pr, state: 'error', title: '', mergeable: null, reviewDecision: null, checks: 'error' },
