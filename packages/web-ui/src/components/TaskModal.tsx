...
const STATUS_META: Record<
	string,
	{ label: string; icon: string; color: string; bg: string; dotDone: string; dotFail: string }
> = {
	todo: {
		label: 'todo',
		icon: '\uD83D\uDCCB',
		...
		bg: 'bg-indigo-500/20',
		dotDone: 'border-green-500 text-green-500',
		dotFail: '',
	},
	in_progress: {
		label: 'in_progress',
	...
	testing: {
		...
		bg: 'bg-violet-500/20',
		dotDone: 'border-green-500 text-green-500',
		dotFail: 'border-red-500 text-red-500',
	},
	fixing: {
		label: 'fixing',
		icon: '\uD83D\uDD27',
...
function buildStageGroups(activities: Activity[], task: Task): StageGroup[] {
	...
	const firstFrom =
		statusChanges.length > 0 ? String(statusChanges[0].payload?.from ?? 'todo') : task.status;
	stages.push({
		status: firstFrom,
		startTime: task.created_at,
		endTime:
			statusChanges.length > 0 ? statusChanges[0].created_at : sorted[sorted.length - 1].created_at,
	});
	...
	for (let i = 0; i < statusChanges.length; i++) {
		...
		const endTime =
			i + 1 < statusChanges.length
				? statusChanges[i + 1].created_at
				: (task.completed_at ?? sorted[sorted.length - 1].created_at);
	...
	return stages.map((stage) => {
		...
		const testFailCount = stageActivities.filter(
			(a) =>
				(a.action === 'test_run' || a.action === 'workflow_test') &&
				a.payload &&
				(a.payload.summary as Record<string, unknown>)?.fail,
		).length;
		const fixCount = stageActivities.filter(
			(a) => a.action === 'create_fix' || a.action === 'workflow_fix_complete',
		).length;

		const durationSeconds = Math.max(0, Math.floor((stageEnd - stageStart) / 1000));
		const passed =
			stage.status === 'done' ||
			stageActivities.some(
				(a) => a.action === 'status_change' && String(a.payload?.from) === stage.status,
			);
...
function Pill({
	variant,
	children,
}: { variant: 'pass' | 'fail' | 'fix' | 'done'; children: React.ReactNode }) {
	const styles = {
	...
	return (
		<span
			className={`text-[10px] font-mono font-semibold px-2 py-0.5 rounded-xl border ${styles[variant]}`}
		>
			{children}
		</span>
	);
...
function ActivityContent({ activity }: { activity: Activity }) {
	...
	switch (action) {
		case 'status_change': {
			...
			const durationSec =
				typeof payload.duration_seconds === 'number' ? payload.duration_seconds : null;
			...
			const depCount =
				typeof payload.dependency_count === 'number' ? payload.dependency_count : null;
			const epicProgress =
				typeof payload.epic_progress === 'object' && payload.epic_progress
					? (payload.epic_progress as Record<string, unknown>)
					: null;
			const totalDurSec =
				typeof payload.total_duration_seconds === 'number' ? payload.total_duration_seconds : null;
			const statusHistory = Array.isArray(payload.status_history)
				? (payload.status_history as Record<string, unknown>[])
				: null;
			const testRunCount =
				typeof payload.test_run_count === 'number' ? payload.test_run_count : null;
			const fixAttemptCount =
				typeof payload.fix_attempt_count === 'number' ? payload.fix_attempt_count : null;
				...
				<div className="mt-2.5">
					<div className="inline-flex items-center gap-2 bg-[#1A1D2E] border border-[#2e3348] rounded-md px-3 py-1.5 font-mono text-xs">
						...
						{durationSec != null && durationSec > 0 && (
							<span className="text-[10px] text-[#555e7a] ml-1">
					...
					{(ticketCode ||
						...
						depCount != null ||
						testRunCount != null ||
						fixAttemptCount != null ||
						blockedFrom) && (
						<div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 text-[10px] font-mono text-[#555e7a]">
							{ticketCode && (
								<span className="bg-[#252a3a] border border-[#2e3348] px-1.5 py-0.5 rounded text-[#9198b4]">
									{ticketCode}
								</span>
							)}
							{subtaskCount != null && (
								<span>
									{'\uC11C\uBE0C\uD0DC\uC2A4\uD06C'} {subtaskDone != null ? `${subtaskDone}/` : ''}
									{subtaskCount}
								</span>
							)}
							{depCount != null && (
					...
					{epicProgress && (
						<div className="mt-2 flex items-center gap-2 text-[10px] font-mono">
							...
							<span className="text-[#9198b4]">
								{String(epicProgress.completed ?? 0)}/{String(epicProgress.total ?? 0)}
							</span>
					...
					{totalDurSec != null && (
						<div className="mt-1 text-[10px] font-mono text-[#555e7a]">
							{'\uCD1D \uC18C\uC694\uC2DC\uAC04'}:{' '}
							<span className="text-[#9198b4]">{formatDuration(totalDurSec)}</span>
						</div>
					)}
		...
		case 'ai_analysis': {
			...
			return (
				<div className="mt-3 bg-[rgba(108,143,255,0.05)] border border-[rgba(108,143,255,0.18)] border-l-[3px] border-l-[#6c8fff] rounded-lg px-3.5 py-2.5">
					...
					<div className="text-xs text-[#9198b4] leading-relaxed whitespace-pre-wrap">
						{analysis}
					</div>
				</div>
			);
			...
			return (
				<div className="mt-2">
					<div className="flex items-center gap-2 flex-wrap text-xs">
						...
						{ticketCode && (
							<span className="font-mono text-[10px] font-semibold bg-[#252a3a] border border-[#3a4060] text-[#9198b4] px-1.5 py-0.5 rounded">
								{ticketCode}
							</span>
						)}
					...
					{(epicTitle || priority != null || assignee || estimatedHrs != null || createdBy) && (
						<div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 text-[10px] font-mono text-[#555e7a]">
							{epicTitle && (
								<span>
									{'\uC5D0\uD53D'}: <span className="text-[#9198b4]">{epicTitle}</span>
								</span>
							)}
					...
					{description && (
						<p className="text-[10px] text-[#555e7a] mt-1 line-clamp-2 leading-relaxed">
							{description}
						</p>
					)}
				</div>
					...
					{ids && ids.length > 0 && (
						<div className="mt-1 flex flex-wrap gap-1">
							{ids.map((id, i) => (
								<span
									key={i}
									className="text-[10px] bg-[#252a3a] border border-[#2e3348] text-[#9198b4] px-1.5 py-0.5 rounded font-mono"
								>
									{id}
								</span>
							))}
		...
		case 'workflow_test': {
			...
			const results = Array.isArray(payload.results)
				...
				: null;
			const runNumber =
				typeof payload.run_number === 'number'
					? payload.run_number
					: typeof payload.runNumber === 'number'
						? payload.runNumber
						: null;
				...
				<div className="mt-2.5">
					{runNumber != null && (
						<div className="text-[10px] font-mono font-semibold text-[#555e7a] mb-1.5 flex items-center gap-1.5">
							<span className="bg-[#252a3a] border border-[#3a4060] text-[#9198b4] px-1.5 py-0.5 rounded">
								{runNumber}
								{'\uCC28 \uC2E4\uD589'}
							</span>
							{fail > 0 ? (
								<span className="text-red-400">
									{'\u00B7'} {fail}/{pass + fail + skip} {'\uC2E4\uD328'}
								</span>
							) : (
						...
						<div className="flex flex-col gap-1">
							{results.map((r, i) => {
								...
								return (
									<details
										key={i}
										open={isFail && !!outputStr}
										className={`rounded-lg border text-xs ${isFail ? 'border-red-500/20 bg-[#1A1D2E]' : 'border-[#2e3348] bg-[#1A1D2E]'}`}
									>
										<summary
											className={`flex items-center gap-2 px-2.5 py-1.5 ${outputStr ? 'cursor-pointer list-none' : 'list-none'}`}
										>
											<span
												className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold shrink-0 ${isFail ? 'bg-red-500/15 text-red-400' : 'bg-green-500/15 text-green-400'}`}
											>
												{isFail ? '\u2715' : '\u2713'}
											</span>
											<span className="text-[#9198b4] flex-1">{String(r.name ?? '')}</span>
											{r.runner ? (
												<span className="font-mono text-[10px] text-[#555e7a]">
													{String(r.runner)}
												</span>
											) : null}
											{r.duration_ms != null && (
												<span className="font-mono text-[10px] text-[#555e7a] min-w-[36px] text-right">
													{Number(r.duration_ms)}ms
												</span>
											)}
											{outputStr && (
												<span
													className={`font-mono text-[10px] shrink-0 ${isFail ? 'text-red-400/60' : 'text-[#555e7a]'}`}
												>
													output
												</span>
											)}
										...
										{outputStr && (
											<div
												className={`mx-2.5 mb-2 mt-1 rounded border ${isFail ? 'border-red-500/20' : 'border-[#2e3348]'} max-h-[200px] overflow-y-auto`}
											>
												<pre className="font-mono text-[11px] text-[#9198b4] whitespace-pre-wrap p-2 leading-relaxed">
													{outputStr}
												</pre>
											</div>
										)}
		...
		case 'create_fix': {
			...
			return (
				<div className="mt-2.5 bg-[rgba(245,146,90,0.04)] border border-[rgba(245,146,90,0.2)] border-l-[3px] border-l-[#f5925a] rounded-lg px-3.5 py-2.5">
					<div className="text-[9px] font-bold uppercase tracking-wider text-[#f5925a] mb-1.5">
						{'\uD83E\uDD16 \uC218\uC815 \uC774\uC720'}
					</div>
					{description && (
						<div className="text-xs text-[#9198b4] leading-relaxed whitespace-pre-wrap">
					...
					{fixTaskId && (
						<div className="mt-1 text-[10px] text-[#555e7a]">
							{'\uC218\uC815 \uD0DC\uC2A4\uD06C:'}{' '}
							<span className="font-mono text-orange-300">{fixTaskId}</span>
						</div>
					)}
					{filesChanged && filesChanged.length > 0 && (
						<div className="mt-1.5 flex flex-wrap gap-1">
							{filesChanged.map((f, i) => (
								<span
									key={i}
									className="text-[10px] bg-[#252a3a] text-[#9198b4] px-1.5 py-0.5 rounded font-mono"
								>
									{f}
								</span>
							))}
		...
		case 'workflow_fix_complete': {
			const notes = typeof payload.notes === 'string' ? payload.notes : null;
			const attemptNumber =
				typeof payload.attempt_number === 'number' ? payload.attempt_number : null;
			return (
		...
		case 'code_change': {
			...
			return (
				<div className="mt-2.5">
					...
					{files && files.length > 0 && (
						<div className="bg-[#0f1117] border border-[#2e3348] rounded-lg overflow-hidden">
							{files.map((f, i) => (
								<div
									key={i}
									className="flex items-center gap-2 px-3 py-1.5 text-[11px] font-mono border-b border-[#2e3348] last:border-b-0"
								>
									<span className="text-[#9198b4] font-semibold flex-1">
										{String(f.path ?? '')}
									</span>
									{f.additions != null && (
										<span className="text-green-400">+{Number(f.additions)}</span>
									)}
									{f.deletions != null && (
										<span className="text-red-400">-{Number(f.deletions)}</span>
									)}
								</div>
							))}
						</div>
					...
					{diffSummary && (
						<pre className="mt-2 bg-[#0f1117] border border-[#2e3348] rounded-lg p-3 text-[11px] font-mono text-[#9198b4] leading-relaxed overflow-x-auto whitespace-pre-wrap">
							{diffSummary}
						</pre>
					)}
				</div>
				...
				<div className="mt-2.5 bg-[#1A1D2E] border border-[#2e3348] rounded-lg px-3 py-2.5 font-mono text-[11px] leading-relaxed">
					...
					{(additions != null || deletions != null) && (
						<div className="text-green-400 mt-0.5">
							...
							{filesChanged && (
								<span className="text-[#555e7a] ml-1">
		...
		case 'create_issue': {
			...
			const issueNumber =
				typeof payload.issueNumber === 'number' || typeof payload.issueNumber === 'string'
					? payload.issueNumber
					: null;
		...
		case 'update': {
			...
			const changes =
				typeof payload.changes === 'object' && payload.changes && !Array.isArray(payload.changes)
				...
				<div className="mt-2 space-y-1">
					...
					{field && (
						<div className="text-xs font-mono text-[#9198b4]">
							<span className="text-[#555e7a]">{field}:</span> {String(payload.from ?? '\u2014')}{' '}
							<span className="text-[#555e7a]">{'\u2192'}</span> {String(payload.to ?? '\u2014')}
						</div>
					)}
						...
						Object.entries(changes).map(([key, val]) => {
							...
							return (
								<div key={key} className="text-xs font-mono text-[#9198b4]">
									<span className="text-[#555e7a]">{key}:</span> {String(fromVal ?? '\u2014')}{' '}
									<span className="text-[#555e7a]">{'\u2192'}</span> {String(toVal ?? '\u2014')}
								</div>
							);
						})}
				</div>
				...
				<div className="mt-2.5">
					{notes && (
						<details open className="rounded-lg border border-green-500/20 bg-[#1A1D2E]">
							<summary className="flex items-center gap-2 px-2.5 py-1.5 cursor-pointer list-none text-xs">
								<span className="w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold shrink-0 bg-green-500/15 text-green-400">
									{'\u2713'}
								</span>
								<span className="text-[#9198b4] flex-1">
									{'\uCF54\uB4DC \uB9AC\uBDF0 \uACB0\uACFC'}
								</span>
							</summary>
					...
					{epicProgress && (
						<div className="mt-1.5 text-[10px] font-mono text-[#555e7a]">
							{'\uC5D0\uD53D \uC9C4\uD589\uB960'}: {String(epicProgress.completed ?? 0)}/
							{String(epicProgress.total ?? 0)} ({String(epicProgress.rate ?? 0)}%)
						</div>
					)}
				</div>
			);
		...
		case 'workflow_start': {
			...
			return (
				<div className="mt-2 flex items-center gap-3 text-xs font-mono text-[#9198b4]">
					{ticketCode && (
						<span className="text-[10px] font-semibold bg-[#252a3a] border border-[#3a4060] px-1.5 py-0.5 rounded">
							{ticketCode}
						</span>
					)}
					{subtaskCount != null && (
		...
		case 'automation_triggered': {
			...
			const rulesTriggered = Array.isArray(payload.rules_triggered)
				? (payload.rules_triggered as string[])
				: null;
			const actionsTaken = Array.isArray(payload.actions_taken)
				? (payload.actions_taken as Record<string, unknown>[])
				: null;
			return (
					...
					{rulesTriggered && rulesTriggered.length > 0 && (
						<div className="flex flex-wrap gap-1 mb-1.5">
							{rulesTriggered.map((rule, i) => (
								<span
									key={i}
									className="text-[10px] font-mono bg-slate-500/10 border border-slate-500/20 text-slate-300 px-1.5 py-0.5 rounded"
								>
									{rule}
								</span>
							))}
					...
					{actionsTaken && actionsTaken.length > 0 && (
						<div className="space-y-0.5">
							{actionsTaken.map((act, i) => {
								...
								const actConfig =
									typeof act.config === 'object' && act.config
								...
								return (
									<div key={i} className="text-[10px] font-mono text-[#555e7a]">
										...
										{actConfig && Object.keys(actConfig).length > 0 && (
											<span className="ml-1 text-[#555e7a]">
												(
				...
				<div className="mt-2 space-y-0.5">
					{Object.entries(payload).map(([key, val]) => (
						<div key={key} className="flex gap-2 text-xs">
							...
							<span className="text-[#9198b4] break-all">
...
function StageCard({
	...
	return (
		...
		>
			...
			>
				<div
					className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm shrink-0 ${iconBgMap[stage.status] ?? iconBgMap.todo}`}
				>
					{stage.icon}
				</div>
				<span className="text-[13px] font-semibold font-mono text-[#e8ecf4]">{stage.label}</span>
				<span className="text-[10px] font-mono text-[#555e7a]">
					{formatTimeRange(stage.startTime, stage.endTime)}
				</span>
				...
				{stage.status === 'testing' && stage.testFailCount === 0 && stage.passed && (
					<Pill variant="pass">{'\uC804\uCCB4 \uD1B5\uACFC'}</Pill>
				)}
				<span
					className={`text-[#555e7a] text-[10px] ml-auto shrink-0 transition-transform ${isOpen ? 'rotate-180' : ''}`}
				>
					{'\u25BE'}
				</span>
			</div>
			...
			>
				<div className="px-4 pb-4 border-t border-[#2e3348]">
					...
					{contentActivities.length === 0 && statusChanges.length === 0 && (
						<div className="text-xs text-[#555e7a] mt-3 font-mono">
							{'\uAE30\uB85D\uB41C \uD65C\uB3D9 \uC5C6\uC74C'}
						</div>
					)}
				</div>
...
/* ─────────────────────────────────────────────
   Stage Flow Bar
───────────────────────────────────────────── */
function StageFlowBar({
	stages,
	onStageClick,
}: { stages: StageGroup[]; onStageClick: (index: number) => void }) {
	return (
			...
			{stages.map((stage, i) => {
				...
				return (
					<div key={i} className="flex items-start shrink-0">
						...
						>
							<div
								className={`w-[26px] h-[26px] rounded-full flex items-center justify-center text-[10px] font-mono font-bold border-[1.5px] shrink-0 ${
									hasFails
										? 'border-red-500 text-red-500 bg-red-500/15'
										: isDone
											? 'border-green-500 text-green-500 bg-green-500/15'
											: 'border-[#3a4060] text-[#555e7a] bg-transparent'
								}`}
							>
								{hasFails ? '\u2715' : isDone ? '\u2713' : i + 1}
							</div>
	...
	useEffect(() => {
		async function load() {
			try {
				...
				const stageCount = buildStageGroups(
					...
					<>
						...
						<div className="px-6 pt-5 pb-0 shrink-0">
							<div className="flex items-center gap-2.5 mb-2.5">
								...
								<span className="font-mono text-[11px] font-semibold text-[#555e7a]">
									P{task.priority}
								</span>
								<span className="text-xs text-[#555e7a]">
							...
							<h2 className="text-xl font-semibold text-[#e8ecf4] leading-tight tracking-tight mb-4">
								{task.title}
							</h2>
						</div>
						...
						<div className="grid grid-cols-3 gap-0 px-6 py-3.5 border-y border-[#2e3348] shrink-0">
							...
							{task.completed_at && (
								<MetaItem
									label="COMPLETED"
									value={new Date(task.completed_at).toLocaleDateString('ko-KR')}
									className="mt-2.5"
								/>
							)}
							{totalDuration > 0 && (
						...
						<div
							...
							style={{ scrollbarWidth: 'thin', scrollbarColor: '#3a4060 transparent' }}
						>
							{/* Description */}
							{task.description && (
								...
								<div className="mb-5">
									...
									<div className="flex flex-col gap-1">
										{subtasks.map((sub) => (
											<div
												key={sub.id}
												className="flex items-center gap-2 px-2.5 py-1.5 bg-[#1A1D2E] border border-[#2e3348] rounded-md text-xs text-[#9198b4]"
											>
												<div
													className={`w-4 h-4 rounded flex items-center justify-center shrink-0 ${
														sub.status === 'done'
															? 'bg-green-500'
															: 'border-[1.5px] border-[#3a4060]'
													}`}
												>
													{sub.status === 'done' && (
												...
												<span
													className={sub.status === 'done' ? 'text-[#555e7a] line-through' : ''}
												>
													{sub.title}
												</span>
												{sub.ticket_code && (
													<span className="ml-auto font-mono text-[9px] text-[#555e7a]">
														{sub.ticket_code}
													</span>
												)}
											</div>
							...
							{fixHistory &&
								(fixHistory.testRuns.length > 0 || fixHistory.fixAttempts.length > 0) && (
									<div className="mt-5">
										<div className="text-[11px] font-semibold text-[#555e7a] uppercase tracking-wider mb-3">
											Test & Fix History
										</div>
										<div className="bg-[#1A1D2E] rounded-lg p-4 space-y-3 border border-[#2e3348]">
											...
											{fixHistory.testRuns.slice(0, 5).map((run) => (
												<div key={run.id} className="flex items-center gap-2 text-xs">
													<span
														className={`w-1.5 h-1.5 rounded-full ${run.status === 'pass' ? 'bg-green-500' : 'bg-red-500'}`}
													/>
													<span className="text-[#555e7a] font-mono">#{run.run_number}</span>
													<span className="text-[#9198b4]">{run.test_type}</span>
													<span
														className={run.status === 'pass' ? 'text-green-400' : 'text-red-400'}
													>
														{run.status}
													</span>
													<span className="text-[#555e7a] font-mono ml-auto text-[10px]">
											...
											{fixHistory.fixAttempts.map((attempt) => (
												<div key={attempt.id} className="bg-[#252a3a] rounded-md p-3 text-xs">
													<div className="flex items-center gap-2 mb-1">
														<span
															className={`w-1.5 h-1.5 rounded-full ${attempt.result_status === 'pass' ? 'bg-green-500' : 'bg-red-500'}`}
														/>
														<span className="text-[#9198b4] font-mono font-medium">
															Attempt #{attempt.attempt_number}
														</span>
														<span
															className={
																attempt.result_status === 'pass' ? 'text-green-400' : 'text-red-400'
															}
														>
													...
													{attempt.fix_description && (
														<p className="text-[#9198b4] ml-3.5">{attempt.fix_description}</p>
													)}
												</div>
											))}
										</div>
...
function MetaItem({
	...
	return (
		<div className={className}>
			<div className="text-[10px] font-semibold uppercase tracking-wider text-[#555e7a] mb-1">
				{label}
			</div>
			<div
				className={`text-[13px] font-medium ${highlight ? 'text-orange-400' : 'text-[#9198b4]'}`}
			>
				{value}
			</div>
		</div>
