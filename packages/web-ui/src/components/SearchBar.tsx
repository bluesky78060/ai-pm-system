		...
		<div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 space-y-3">
			...
			{showAdvanced && (
				<div className="space-y-3 border-t border-slate-700 pt-3">
					{/* Status Filter */}
					<div>
						<span className="block text-slate-400 text-sm mb-2">상태</span>
						<div className="flex flex-wrap gap-2">
							{['todo', 'in_progress', 'testing', 'fixing', 'review', 'done', 'blocked'].map(
								(status) => (
									<button
										key={status}
					...
					{/* Priority Filter */}
					<div>
						<span className="block text-slate-400 text-sm mb-2">우선순위</span>
						<div className="flex gap-2">
					...
					{/* Assignee Filter */}
					<div>
						<span className="block text-slate-400 text-sm mb-2">담당자</span>
						<div className="flex gap-2">
					...
					{epics.length > 0 && (
						<div>
							<span className="block text-slate-400 text-sm mb-2">에픽</span>
							<div className="flex flex-wrap gap-2">
								{epics.map((epic) => (
									<button
										key={epic.id}
