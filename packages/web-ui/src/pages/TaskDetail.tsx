...
export default function TaskDetail() {
    ...
    async function load() {
      try {
        const [taskData, historyData] = await Promise.all([
          api.getTask(id!),
          api.getFixHistory(id!).catch(() => null),
        ]);
  ...
  return (
    <div>
      <Link to="/" className="text-sm text-gray-500 hover:text-gray-300 mb-4 inline-block">&larr; Back</Link>

      {/* Task header */}
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-6 mb-6">
        <div className="flex items-start justify-between mb-4">
          <h1 className="text-xl font-bold">{task.title}</h1>
          <span className={`text-xs px-3 py-1 rounded-full ${STATUS_COLORS[task.status] ?? 'bg-gray-600'} text-white`}>
            {task.status}
          </span>
        </div>
        {task.description && (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <div className="font-medium">{new Date(task.created_at).toLocaleDateString('ko-KR')}</div>
          </div>
          <div>
        ...
        <div className="flex gap-3 mt-4">
          {task.github_pr && (
            <a href={task.github_pr} target="_blank" rel="noreferrer" className="text-xs text-blue-400 hover:underline">
              PR: {task.github_pr.split('/').pop()}
            </a>
          )}
          {task.github_issue && (
            <a href={task.github_issue} target="_blank" rel="noreferrer" className="text-xs text-blue-400 hover:underline">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {subtasks.map((sub) => (
              ...
              >
                <span className={`w-2 h-2 rounded-full shrink-0 ${STATUS_COLORS[sub.status] ?? 'bg-gray-600'}`} />
                <span className="flex-1 text-sm">{sub.title}</span>
                <span className={`text-xs px-2 py-0.5 rounded-full ${STATUS_COLORS[sub.status] ?? 'bg-gray-600'} bg-opacity-20 text-gray-300`}>
                  {sub.status}
                </span>
              </Link>
            ))}
              ...
              <div className="space-y-2 mb-6">
                ...
                {fixHistory.testRuns.slice(0, 10).map((run) => (
                  <div key={run.id} className="flex items-center gap-3 text-sm">
                    <span className={`w-2 h-2 rounded-full ${run.status === 'pass' ? 'bg-green-500' : run.status === 'fail' ? 'bg-red-500' : 'bg-gray-500'}`} />
                    ...
                    <span className="text-gray-600 text-xs ml-auto">{new Date(run.created_at).toLocaleString('ko-KR')}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {fixHistory.fixAttempts.length > 0 && (
              <div className="space-y-2">
                ...
                {fixHistory.fixAttempts.map((attempt) => (
                  <div key={attempt.id} className="bg-gray-800 border border-gray-700 rounded p-3 text-sm">
                    <div className="flex items-center gap-3 mb-1">
                      <span className={`w-2 h-2 rounded-full ${attempt.result_status === 'pass' ? 'bg-green-500' : attempt.result_status === 'fail' ? 'bg-red-500' : 'bg-orange-500'}`} />
                      <span className="text-gray-300 font-medium">Attempt #{attempt.attempt_number}</span>
                      <span className={`text-xs ${attempt.result_status === 'pass' ? 'text-green-400' : attempt.result_status === 'fail' ? 'text-red-400' : 'text-orange-400'}`}>
                        {attempt.result_status}
                      </span>
                      <span className="text-gray-600 text-xs ml-auto">{new Date(attempt.created_at).toLocaleString('ko-KR')}</span>
                    </div>
                    ...
                    {attempt.files_changed && Array.isArray(attempt.files_changed) && attempt.files_changed.length > 0 && (
                      <div className="mt-2 ml-5 flex flex-wrap gap-1">
                        {attempt.files_changed.map((file: string, i: number) => (
                          <span key={i} className="text-xs bg-gray-700 text-gray-300 px-2 py-0.5 rounded font-mono">{file}</span>
                        ))}
                      </div>
                    )}
                  </div>
