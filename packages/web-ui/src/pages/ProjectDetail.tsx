...
import { api, type ProjectStatus, type BlockingAnalysis, type Task, type Epic } from '../api';
        ...
        <div className="flex items-center gap-5">
          {Object.entries(breakdown).map(([key, count]) => (
            <div key={key} className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-slate-400 capitalize">{key.replace('_', ' ')}: {count}</span>
            </div>
          ))}
        </div>
      </div>
